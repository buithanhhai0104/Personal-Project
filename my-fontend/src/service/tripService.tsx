...
export const getAllTrips = async () => {
  try {
    const response = await axios.get(
      "https://server-personal-project-dgoctsqto-thanh-hais-projects-0e39a8d1.vercel.app/api/trips"
    );
...
export const deleteTrip = async (id: number) => {
  try {
    const response = await axios.delete(
      `https://server-personal-project-dgoctsqto-thanh-hais-projects-0e39a8d1.vercel.app/api/trips/${id}`,
      {
        withCredentials: true,
      }
    );
...
export const apiTripsSearch = async (params: IParams) => {
  try {
    const response = await axios.get(
      "https://server-personal-project-dgoctsqto-thanh-hais-projects-0e39a8d1.vercel.app/api/search",
      {
        params,
      }
    );

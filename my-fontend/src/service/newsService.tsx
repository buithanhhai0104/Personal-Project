...
export const getNewsAll = async () => {
  try {
    const res = await axios.get(
      "https://https://server-personal-project-git-e4d4fd-thanh-hais-projects-0e39a8d1.vercel.app/news"
    );
    return res.data;
  } catch (err) {
    console.log("Lỗi lấy dữ liệu bài báo theo id", err);
...
export const createNews = async (newsData: INews) => {
  try {
    const res = await axios.post(
      "https://server-personal-project-git-e4d4fd-thanh-hais-projects-0e39a8d1.vercel.app/news",
      newsData,
      {
        headers: {
